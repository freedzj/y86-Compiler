...
{
    if (srclow > 64 || dstlow > 64 || length > 64 || ((srclow + length) > 64) || ((dstlow + length) > 64))
        return dest;
    else
    {
        uint64_t temp = 0;
        temp = clearBits(dest, dstlow, ((length - 1) + dstlow));
        //std::cout << std::hex << temp << "\n";
        uint64_t temp1 = getBits(source, srclow, ((length - 1) + srclow));
        //std::cout << "Temp1: " << std::hex << temp1 << "\n";
        temp |= (temp1 << dstlow);
        //std::cout << std::hex << temp << "\n";
        return temp;
    }
...
{
    return ((sign(op1) == sign(op2)) && (sign(op1 + op2) != sign(op1)));
}
